// components/RegistrationFeeItemForm.tsx
import { useForm } from 'react-hook-form';
import { zodResolver } from '@hookform/resolvers/zod';
import { createRegistrationFeeItemSchema } from '@/lib/validators';
import type { CreateRegistrationFeeItemFormData } from '@/lib/validators';
import type { RegistrationFeeItem } from '@/types/api';

interface RegistrationFeeItemFormProps {
    initialData?: RegistrationFeeItem;
    onSubmit: (data: CreateRegistrationFeeItemFormData) => Promise<void>;
    isSubmitting: boolean;
}

export default function RegistrationFeeItemForm({ initialData, onSubmit, isSubmitting }: RegistrationFeeItemFormProps) {
    const {
        register,
        handleSubmit,
        formState: { errors },
    } = useForm<CreateRegistrationFeeItemFormData>({
        resolver: zodResolver(createRegistrationFeeItemSchema),
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input
                        ...
                        {...register('category')}
                        className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-gray-100"
                        placeholder="e.g., tuition, library, medical"
                    />
                    {errors.category && (
                ...
                <div className="md:col-span-2 space-y-4">
                    <div className="flex items-center">
                        ...
                        <label htmlFor="is_required" className="ml-2 block text-sm text-gray-900 dark:text-gray-100">
                            Required (Students must pay this fee)
                        </label>
                    </div>
            ...
            <div className="flex justify-end space-x-3 pt-6 border-t border-gray-200 dark:border-gray-700">
                ...
                >
                    {isSubmitting ? 'Saving...' : initialData ? 'Update Fee Item' : 'Create Fee Item'}
                </button>
            </div>
        </form>
    );
