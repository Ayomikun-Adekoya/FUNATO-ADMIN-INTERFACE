// pages/admin/registration-fee-items/index.tsx
...
import { useRegistrationFeeItems, useDeleteRegistrationFeeItem } from '@/lib/queries';
import { formatDate } from '@/utils/date';
import { formatCurrency } from '@/utils/format';
import type { RegistrationFeeItem } from '@/types/api';

export default function RegistrationFeeItemsListPage() {
    const [page, setPage] = useState(1);
    const [perPage, setPerPage] = useState(15);
    const [search, setSearch] = useState('');
    const [isActiveFilter, setIsActiveFilter] = useState('');
    const [isRequiredFilter, setIsRequiredFilter] = useState('');
    const [categoryFilter, setCategoryFilter] = useState('');
    const [deleteModalOpen, setDeleteModalOpen] = useState(false);
    const [itemToDelete, setItemToDelete] = useState<RegistrationFeeItem | null>(null);

    const { data, isLoading } = useRegistrationFeeItems({
        page,
        per_page: perPage,
        search,
        is_active: isActiveFilter === '' ? undefined : isActiveFilter === 'true',
        is_required: isRequiredFilter === '' ? undefined : isRequiredFilter === 'true',
        category: categoryFilter || undefined,
    });

    const deleteMutation = useDeleteRegistrationFeeItem();
    ...
    const columns = [
        {
            key: 'name',
            header: 'Fee Item',
            render: (item: RegistrationFeeItem) => (
                <div>
        ...
        },
        {
            ...
            render: (item: RegistrationFeeItem) => (
                <span
                    className={`px-2 py-1 text-xs font-semibold rounded-full ${item.is_required
        ...
        },
        {
            ...
            render: (item: RegistrationFeeItem) => (
                <div className="flex space-x-2">
                    <Link
                        href={`/admin/registration-fee-items/${item.id}`}
        ...
        <ProtectedRoute>
            <Layout>
                <div className="space-y-6">
                    ...
                    <div className="flex justify-between items-center">
                        <div>
                            ...
                            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                                Manage registration fee items for student enrollment
                            </p>
                        </div>
                        <Link
                            href="/admin/registration-fee-items/create"
                    ...
                    <div className="bg-white dark:bg-gray-800 shadow rounded-lg p-4">
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
                            ...
                            {/* Required Filter */}
                            <div>
                                <label htmlFor="required" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                                    Required
                                </label>
                                <select
                                    id="required"
                                    value={isRequiredFilter}
                                    onChange={(e) => setIsRequiredFilter(e.target.value)}
                                    className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-gray-100"
                                >
                ...
                <Modal
                    ...
                    title="Delete Registration Fee Item"
                >
